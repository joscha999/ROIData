...
namespace ROIData {
	public static class TaskSystem {
		...
		private static readonly Dictionary<int, AssignmentTask> AllTasks = new Dictionary<int, AssignmentTask>();

		private static readonly Dictionary<AssignmentActionType, CustomStaticEvent> EventTypeCustomEventPairs
			= new Dictionary<AssignmentActionType, CustomStaticEvent>();

		public static readonly TimeManager TimeManager = ManagerBehaviour<TimeManager>.instance;
		...
		public static void ReceiveTasks(List<AssignmentTask> tasks) {
			foreach (var task in tasks) {
				if (AllTasks.TryGetValue(task.ID, out var existingTask)) {
					...
					existingTask.UTCStart = task.UTCStart;
				} else {
		...
		private static void UpdateTasks() {
			if (CurrentTask == null) {
				...
				//check if we reached the end of a task (unix days wise)
				if (CurrentTask.UnixDayEnd <= Date.Now.UnixDays) {
					PauseGame();
					CurrentTask = null;
				}
			}
		...
		private static void Research(ResearchControlParameters parameters) {
